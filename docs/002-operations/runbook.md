...
- The official identity worker runs at `15 18 * * 1-5` UTC and once at worker startup. It ingests official company, ETF, ETN, company-delisting, and ETN-retirement sources for both TWSE and TPEx into append-only canonical history. The TPEx company-delisting import covers each supported year from 2021 through the acquisition year. ETNs use the venues' explicit retirement tables; after a successful venue ETF snapshot, previously active ETFs absent from that current feed receive inactive revisions. These lifecycle inputs prevent retired products and corrected or reused tickers from resolving as current identities.
