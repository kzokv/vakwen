...
- The official identity and monthly-revenue worker runs at `15 18 * * 0-4` UTC (02:15 Monday-Friday in Taiwan) and once at worker startup. It ingests official company, ETF, ETN, company-delisting, ETN-retirement, and MOPS monthly-revenue sources for both TWSE and TPEx into append-only canonical history. Identity lifecycle inputs remain fail-closed for empty or incomplete snapshots. Revenue acquisition preserves raw ROC month/date values, publication context, source comparisons, basis and qualifier signals, and immutable provenance per retrieval.
