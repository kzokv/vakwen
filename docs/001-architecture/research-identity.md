...
4. Memory and PostgreSQL append immutable revisions. Reads apply both `effectiveAt` and `knowledgeAt` cutoffs. Once effective and known, an explicit inactive status-only revision has terminal precedence over full snapshots for the same immutable Listing, including lagging active snapshots with later effective dates. A blocked current row still contributes its non-status identity observations through an inactive identity basis, while the retirement source remains the sole status authority. Scheduled acquisition reads one bounded latest revision per Listing and venue rather than materializing each Listing's complete immutable history.
