...
- Codex migration follow-up makes the portfolio-initialization backfill conditional on the marker column's first creation, so rerunning migration 114 cannot initialize users created afterward. The managed PostgreSQL migration regression passed, including clean-schema, existing-user backfill, and post-migration user rerun behavior; targeted API lint and typecheck were clean.
