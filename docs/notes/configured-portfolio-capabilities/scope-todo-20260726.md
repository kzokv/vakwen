...
- Codex reliability follow-up makes reporting-currency response enrichment non-failing after account create/update commits. When preferences are temporarily unavailable, the response preserves the committed account result, reports the requested currency as unknown, and derives a safe effective currency from canonical capabilities; focused route coverage passed 11/11 with targeted API lint and typecheck clean.
- Codex race/reliability follow-up marks invalidated shell-config refresh results as stale so SSE callbacks cannot overwrite newer authoritative mutation capabilities, and extends non-failing reporting-currency enrichment to delegated delete, restore, and purge responses. Focused shell-hook coverage passed 10/10 and account mutation route coverage passed 12/12 with targeted lint plus web/API typecheck clean.
- Codex zero-account/shared-context follow-up hides all operational Quick Actions while capabilities are unknown or empty, leaving the setup gate as the only action, and preserves the active shared-owner header when the E2E account wizard verifies its committed account. Focused Quick Actions coverage passed 5/5 and test-E2E identity coverage passed 2/2 with targeted lint, web typecheck, and test-E2E build clean.
- Codex migration follow-up makes the portfolio-initialization backfill conditional on the marker column's first creation, so rerunning migration 114 cannot initialize users created afterward. Its reporting-currency normalization now also upserts preferences for existing active-account owners whose lazy preferences row or key never existed, preventing a non-TWD-only portfolio from retaining an implicit TWD server default. The managed PostgreSQL migration regression passed, including clean-schema, stored/implicit/missing preference backfills, and post-migration user rerun behavior; targeted API lint and typecheck were clean.
